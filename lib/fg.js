  ...
  */
  ...
  process.on("exit",function(){
    ...
    var tosave = Object.keys(self.store.cache).filter(function(key){
      return self.store.cache[key].mode.match(/dynamic/ig)
    })
...
FileGateway.prototype.__getContentSync = function(obj, filename, ops) {
  ...
  if (self.store.cache[filename] !== undefined && self.store.cache[filename] !== null)
    return self.store.cache[filename].content

  // TODO: write file default if not exists
  var content = fs.readFileSync(filename, ops.encoding)

  if(obj.json === true || ops.json == true)
